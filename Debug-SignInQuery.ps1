...
# ==== Test 5: Rohes JSON der ersten Antwort (Test 1) für Detailanalyse ====
Write-Output "=== TEST 5: Rohes erstes Sign-in Objekt (aus Test 1) ==="
...
# ==== Test 6: Vergleich userId aus Gruppe vs. aus Sign-in Log ====
Write-Output "=== TEST 6: userId-Vergleich ==="
...
Write-Output "  Test 1 (alle, userId):              $count1 Sign-in(s)"
...
if ($count1 -eq 0 -and $count3 -gt 0) {
    ...
    Write-Output "     Wahrscheinlich: Fehler haben errorCode=0 (CA-Failure / Interrupted)."
} elseif ($count1 -eq 0 -and $count3 -eq 0 -and $count4 -gt 0) {
    Write-Output "  => DIAGNOSE: Nur Beta-Endpoint liefert Ergebnisse."
    Write-Output "     Wahrscheinlich: Sign-in Typ ist in v1.0 nicht enthalten."
